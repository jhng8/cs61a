...
(define (accumulate merger start n term)
    (cond
      ...
      (else (accumulate merger (merger (term n) start) (- n 1) term)))
)

; Question 5
; 
(define (without-duplicates lst) 
  (define (helper old_lst curr_lst)
    (define (check? num l)
      (cond 
        ((null? l) #f)
        ((= num (car l)) #t)
        (else (check? num (cdr l)))))
    (cond 
      ((null? old_lst) curr_lst)
      ((check? (car old_lst) curr_lst) (helper (cdr old_lst) curr_lst))
      (else (helper (cdr old_lst) (cons (car old_lst) curr_lst))))
  )
  (define (reverse lst2 reversed)
    (if (null? lst2) reversed
      (reverse (cdr lst2) (cons (car lst2) reversed)))
  )
  (reverse (helper lst nil) nil)
)
