...
def piggy_points(score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    absDiff = abs(score%10 - (score//10)%10)
    return 4 + absDiff
    # END PROBLEM 2
...
def more_boar(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    playerMax, playerMin, oppMax, oppMin = 0, 10, 0, 10
    flag1 = True
    flag2 = True
    while player_score != 0 or flag1:
        digit = player_score % 10
        playerMax = max(playerMax, digit)
        playerMin = min(playerMin, digit)
        player_score //= 10
        flag1 = False
    while opponent_score != 0 or flag2:
        digit = opponent_score % 10
        oppMax = max(oppMax, digit)
        oppMin = min(oppMin, digit)
        opponent_score //= 10
        flag2 = False
    if playerMax > oppMax and playerMin < oppMin:
        return True
    return False 
    # END PROBLEM 4
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    dice_num = 1
    for i in range(1,11):
        curr_score = make_averaged(roll_dice, trials_count)(i,dice)
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if more_boar(new_score,opponent_score):
        return 0
