...
def about(topic):
    ...
    def helper(paragraph):
        new_para = lower(remove_punctuation(paragraph)).split()
        for i in new_para:
            if i in topic:
                return True
        return False
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        ...
        for i in valid_words:
            new_min = diff_function(typed_word,i,limit)
            if new_min < min_diff:
                min_diff = new_min
                value = i
        if min_diff > limit:
...
def feline_flips(start, goal, limit):
    ...
    def helper(s, g, count):
        if len(s)==0 or count>limit:
            return 0
        else:
            if s[-1]!=g[-1]:
                return 1+helper(s[:-1],g[:-1],count+1)
            else:
                return helper(s[:-1],g[:-1],count)
    return helper(start,goal,0)+diff
    # END PROBLEM 6
...
def minimum_mewtations(start, goal, limit):
    ...
    if start == "" or goal == "":  # Fill in the condition
        # BEGIN
        "*** YOUR CODE HERE ***"
        return abs(len(start) - len(goal)) if abs(len(start) - len(goal)) <= limit else 100000
        # END
        ...
        add = 1 + minimum_mewtations(start, goal[1:], limit -1)  #scats --> 4
        remove = 1 + minimum_mewtations(start[1:], goal, limit - 1)  #ats -- > 0
        substitute = 1 + minimum_mewtations(start[1:], goal[1:], limit - 1)  #sats --> 1
        # BEGIN
        "*** YOUR CODE HERE ***"
...
def fastest_words(game):
    ...
    for j in word_indices:
        for k in player_indices:
            times[k] = time(game, k, j)
        min_index = times.index(min(times))
        lst[min_index].append(word_at(game,j))
