...
class Ant(Insect):
        ...
        if type(self)!= QueenAnt:
            ...
            if self.fake:
                if self.place.ant.is_container:
                    ContainerAnt.remove_ant(self.place.ant,self)
                else:
                    place.ant = None
            else:
                return
    ...
    def buff(self):
        ...
        if self.is_buffed == False:
            self.damage *= 2
        if self.is_container and self.ant_contained and self.ant_contained.is_buffed == False:
        ...
        while not location.is_hive:
            if self.min_range <= transitions <= self.max_range:
                if choose_bee(location.bees) != None:
                    return choose_bee(location.bees)
                else:
                    transitions += 1
                    location = location.entrance
            elif transitions > self.max_range:
                return None
            else:
                transitions += 1
                location = location.entrance
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for bee in bee_copy:
            Insect.reduce_health(bee, reflect)        
...
class HungryAnt(Ant):
    ...
    def action(self,gamestate):
        ...
        elif choose_bee(self.place.bees)!=None:
            lucky_bee = choose_bee(self.place.bees)
...
# END Problem EC
    ...
    def reduce_health(self, amount):
        ...
        if amount> self.health and not self.fake:
            bees_win()
        else:
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.place.ant != None and type(self.place.ant) == QueenAnt and not self.place.ant.fake:
            bees_win()
